...
if [[ -z "$port" || -z "$sdk" || ! -f "$sdk/export.sh" ]]; then
    echo 'Usage: ./flash-local.sh PORT /absolute/path/to/esp-idf' >&2
    exit 1
fi
if [[ ! -e "$port" ]]; then
...
if [[ ! -f "$project_dir/build/hfp_ag.bin" ]]; then
    echo 'Build output is missing; run build-local.sh first.' >&2
    exit 1
fi
...
# `flash` writes only bootloader, partition table, and app. It deliberately
# does not erase or write the NVS partition that contains the Bluetooth bond.
idf.py -C "$project_dir" -B "$project_dir/build" -p "$port" flash
