...
test('board selection shows the release and preserves the exact installer identity', async () => {
  const element = () => ({
  ...
  const choices = ['phone', 'car'].map((value, i) => ({
    value, checked: !i,
    dataset: { manifest: `./${value}.json`, release: '0.3.1-alpha', version: `0.3.1-alpha+${i ? 'bbbbbbbbbbbb' : 'aaaaaaaaaaaa'}` },
  ...
  assert.equal(nodes['#firmware-version'].textContent, `v${choices[0].dataset.release}`);
  choices[0].checked = false; choices[1].checked = true; choices[1].change();
  const selectedInstaller = nodes['#install-control'].child;
  assert.equal(nodes['#firmware-version'].textContent, `v${choices[1].dataset.release}`);
