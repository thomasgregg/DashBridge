...
static void send_media_state() {
    if (send_to_car(music::state_message(media_state))) {
        ++media_updates;
        last_media_send = now();
    } else {
        ++control_failures;
...
static void controller_event(esp_avrc_ct_cb_event_t event, esp_avrc_ct_cb_param_t *param) {
    ...
    case ESP_AVRC_CT_METADATA_RSP_EVT: {
        const size_t length = size_t(std::max(0, param->meta_rsp.attr_length));
        ...
        case ESP_AVRC_MD_ATTR_PLAYING_TIME: {
            uint32_t duration = 0;
            if (music::decimal(value, duration)) media_state.length_ms = duration;
            break;
        }
        default: break;
        }
...
static void target_event(esp_avrc_tg_cb_event_t event, esp_avrc_tg_cb_param_t *param) {
    ...
    case ESP_AVRC_TG_PASSTHROUGH_CMD_EVT:
        if (supported_control(param->psth_cmd.key_code)) {
            const auto message = music::command_message(control_session, ++control_sequence,
...
static void sink_audio(esp_a2d_conn_hdl_t handle, esp_a2d_audio_buff_t *buffer) {
    music::Audio audio;
    bool accepted = false;
    if (buffer && linked && streaming && codec_ready && handle == connection && buffer->data &&
...
#if CONFIG_BRIDGE_CAR
    ++rx_packets;
    if (!linked || !codec_ready || !streaming || !connection || !audio.size ||
...
#if CONFIG_BRIDGE_PHONE
    uint8_t key = 0, state = 0;
    if (!music::parse_command(message, key, state) || !supported_control(key)) {
        ++control_failures;
        return;
    }
    if (message.session != command_session) {
        command_session = message.session;
        last_command = 0;
    }
    if (message.notice.id <= last_command) return;
    last_command = message.notice.id;
    if (!avrc_linked || esp_avrc_ct_send_passthrough_cmd(next_transaction(), key, state) != ESP_OK)
        ++control_failures;
    else
        ++control_commands;
#else
    music::State incoming;
    if (!music::parse_state(message, incoming)) {
        ++control_failures;
        return;
    }
    if (media_state.session == incoming.session && incoming.revision < media_state.revision)
        return;
    const bool track_changed = media_state.session != incoming.session ||
                               media_state.revision != incoming.revision;
    const bool status_changed = media_state.playback != incoming.playback;
    const bool position_changed = media_state.position_ms != incoming.position_ms;
    media_state = std::move(incoming);
    ++media_updates;
    if (track_changed) notify_changed(ESP_AVRC_RN_TRACK_CHANGE);
    if (status_changed) notify_changed(ESP_AVRC_RN_PLAY_STATUS_CHANGE);
    if (position_changed) notify_changed(ESP_AVRC_RN_PLAY_POS_CHANGED);
#endif
}
...
void music_poll() {
    ...
    if (linked && codec_ready && !streaming && !start_pending && now() - connected_at >= 500) {
...
void music_status() {
    ESP_LOGI(tag,
             "Music alpha: profile=%d peer=%d connecting=%d linked=%d codec=%d streaming=%d avrc=%d handle=%u mtu=%u tx=%lu rx=%lu dropped=%lu send_fail=%lu media=%lu controls=%lu control_fail=%lu",
...
void music_start() {
#if CONFIG_BRIDGE_PHONE
    media_state.session = token();
    media_state.revision = 1;
